...
9. ~~**Admin (Organizations / Users / NewOrg)** — al final, son los menos usados.~~ ✅ **Hecho** — Primitivas nuevas en `components/ui/`: [KebabMenu](../../components/ui/kebab-menu.tsx) (menú de tres puntos reutilizable) y [ConfirmSheet](../../components/ui/confirm-sheet.tsx) (confirmación destructiva como `Sheet`), que reemplazan los modales `fixed` hechos a mano. **Organizations** ([organizations/page.tsx](../../app/dashboard/organizations/page.tsx)): sticky header + FAB mobile, [OrganizationCard](../../components/organizations/OrganizationCard.tsx) con tile mesh-info, chip de licencia junto al nombre (Activa / Por vencer ≤7d / En gracia / Vencida / Sin licencia), línea `/slug · dueño · N miembros`, `Vence:` y kebab (Ver detalles / Eliminar). **Users** ([users/page.tsx](../../app/dashboard/users/page.tsx)): tabla → [UserCard](../../components/users/UserCard.tsx) con avatar del color del rol, email, organización y chip de rol uppercase (admin = secondary, owner = primary, staff = info, special = warning); cambio de rol y borrado en el kebab; botón `soft` “Invitar nuevo usuario” abre [InviteUserSheet](../../components/users/InviteUserSheet.tsx). **NewOrg** ([organizations/new/page.tsx](../../app/dashboard/organizations/new/page.tsx)): header con chevron back, `Field` en todo el form, slug con prefijo `turnoflash.com/` auto-generado desde el nombre hasta que se edita a mano, dueño como cards seleccionables (radiogroup), fechas en 2 cols. Lógica intacta: mismas queries, RPC `create_organization_with_owner` y edge function `invite-user`. **Extensión (2026-09-10):** también rediseñadas [organizations/details](../../app/dashboard/organizations/details/page.tsx) (hero con chip de licencia + 3 stats, cards Información/Licencia/Dueño/Miembros con labels uppercase; edición en [OrganizationEditSheet](../../components/organizations/OrganizationEditSheet.tsx) y alta de miembros en [AddMemberSheet](../../components/organizations/AddMemberSheet.tsx)) e [invite](../../app/dashboard/invite/page.tsx) (sticky header, `Field`, pasos numerados). La lista de organizaciones pasó de 1+2N consultas a 2 en paralelo, y el detalle de 3 secuenciales a 2 en paralelo.
