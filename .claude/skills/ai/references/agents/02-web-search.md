...
export async function POST(req: Request) {
  ...
  // Add :online if webSearch is active
...
export function ChatWidget() {
  ...
  const [webSearch, setWebSearch] = useState(false)  // New state
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    // Send with web search option
    sendMessage({
      text,
      body: { webSearch }  // Pass to API route
    })
  }
...
const result = streamText({
  model: openrouter(`${MODELS.balanced}:online`),  // Always online
  system: SYSTEM_PROMPT,
  messages: modelMessages,
})
```
