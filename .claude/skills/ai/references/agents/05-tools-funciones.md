...
export const getWeather = tool({
  ...
  // Optional: Define the output schema for type safety
  ...
  execute: async ({ city }) => {
    // Here would go the call to a weather API
    return {
      city,
...
export const searchProducts = tool({
  ...
  execute: async ({ query, category, maxPrice }) => {
    // Here would go the actual search in Supabase
    return {
      query,
      results: [
...
export async function POST(req: Request) {
  ...
  const result = streamText({
    ...
    tools,
    // AI SDK 5: stopWhen replaces maxSteps
    stopWhen: stepCountIs(5),  // Maximum 5 loop iterations
  })
...
export async function POST(req: Request) {
  ...
  const result = streamText({
    ...
    // AI SDK 5: Dynamic per-step control
    prepareStep: async ({ stepNumber, previousSteps }) => {
      // Example: Change model based on complexity
      if (stepNumber > 2) {
        return {
          model: openrouter(MODELS.powerful),  // Use more powerful model
        }
      }

      // Example: Limit tools after certain steps
      if (stepNumber > 3) {
        return {
      ...
      // Example: Compress messages if there are many
            ...
            <div className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              ...
              >
                ...
                {m.parts?.map((part, i) => {
                  ...
                  // Tool call (the model wants to use a tool)
...
```typescript
// In the component, handle confirmation
...
const result = streamText({
  ...
  // Stop when: 5 steps OR finalAnswer is called
  stopWhen: or(
    stepCountIs(5),
