...
export const MODELS = {
  ...
  vision: 'google/gemini-2.0-flash-exp:free',  // Free, good
  visionPro: 'openai/gpt-4o',                   // Better quality
  visionClaude: 'anthropic/claude-3-5-sonnet',  // Alternative
} as const
```
...
export async function POST(req: Request) {
  ...
  // Detect if there are images in the last message
...
interface UploadedImage {
  id: string
  file: File
  preview: string   // Blob URL to display
  base64: string    // Data URL to send
}
...
export function useImageUpload(): UseImageUploadReturn {
  ...
  // Clean up memory URLs on unmount
  useEffect(() => {
    return () => {
  ...
  useEffect(() => {
    ...
    // Listen on document to capture global paste
