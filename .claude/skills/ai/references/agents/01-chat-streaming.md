...
export async function POST(req: Request) {
  ...
  // Convert UIMessage[] to model format
  ...
  // Return stream compatible with useChat
...
// NEVER MODIFY - Re-exports the SDK hook

'use client'

// SDK v5: import from @ai-sdk/react
...
// MODIFY: Styles according to your design
...
export function ChatWidget() {
  // SDK v5: input is handled externally
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    setInput('')
    // SDK v5: sendMessage expects { text: string }
    sendMessage({ text })
  }
...
```typescript
// app/page.tsx or wherever you need it
...
const result = streamText({
  model: openrouter(MODELS.powerful), // Change here
  // ...
})
```
