...
// NEVER MODIFY - Core of the streaming

/**
 * Parses incomplete JSON by automatically closing brackets.
 * Allows processing responses as they arrive during streaming.
 */
...
// MODIFY: Add your specific actions

import { z } from 'zod'

// === BASE ACTIONS (always include) ===
...
// MODIFY: Only the SYSTEM_PROMPT and available actions
...
// NEVER MODIFY - Core of the client
...
// MODIFY: Customize the rendering of each type
...
// ============================================
// ACTION VALIDATION (avoids empty bubbles)
// ============================================

// MODIFY: Add the types you use here
const VALID_ACTION_TYPES = new Set(['think', 'message', 'ask', 'calculate', 'tool'])

// Helper: validates that an action has content
function isValidAction(action: StreamingAction): boolean {
  // 1. Reject unrecognized types
...
export function ActionFeed({ actions }: Props) {
  return (
    <div className="space-y-3">
      {actions
        .filter(isValidAction) // <-- Filters invalid/empty actions
        .map((action, i) => (
...
function ActionItem({ action }: { action: StreamingAction }) {
  ...
  switch (action._type) {
    ...
    default:
      // Unrecognized type - do not render (the filter should have already caught it)
      console.warn('ActionItem: tipo no reconocido', action._type)
      return null
  }
}

// Minimal toggle for thinking
  ...
  return (
    ...
    >
      {expanded ? (
        ...
        // Collapsed: just "thinking..." in italics
  ...
  return (
    <div className="flex flex-col h-full max-w-2xl mx-auto">
      ...
      <div className="flex-1 overflow-y-auto p-4">
        ...
        {/* Minimal indicator while starting */}
