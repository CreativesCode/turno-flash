...
### Explorar
```
list_tables                    -- View DB structure
execute_sql("SELECT ...")      -- Query data
get_logs(service: "auth")      -- Debug auth
get_logs(service: "postgres")  -- Debug DB
```
...
### Verificar Seguridad
```
get_advisors(type: "security") -- Detects tables without RLS
search_docs("query here")      -- Search official docs
```
...
```
Structure:
  list_tables → design → apply_migration → RLS → policies → get_advisors → test data

Data:
  Load credentials → PostgREST (CRUD) or Management API (SQL) → verify result
```
