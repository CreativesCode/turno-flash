...
```javascript
// Service Worker v1.0
// CRITICAL: do NOT include a fetch handler (breaks iOS Safari PWA)
...
// Click: navigate to the notification URL
...
// Update: force activation of new version
...
// Auto-resubscribe if the browser invalidates the push subscription
...
export default function PWARegister() {
  useEffect(() => {
    ...
    // CRITICAL: use window.location.origin (iOS rejects 307 redirects)
...
export async function POST(request: NextRequest) {
  try {
    ...
    // If this is a subscription change, delete the previous one
    if (oldEndpoint) {
      await supabaseAdmin
...
export async function POST(request: NextRequest) {
  // Auth: only the service role can send
  ...
  try {
    ...
    for (const sub of subscriptions) {
      try {
        ...
        // 4xx = invalid subscription, delete it (except 429 rate limit)
...
// Inside the return, at the end of the body:
<PWARegister />
